...
<head>
  <meta charset="UTF-8">
  <title>Admin - Kelola Owner</title>
  <style>
    :root{
      --bg1:#052b31;
      --bg2:#0a424a;
...
<div class="wrap">
  <div class="card">
    <div class="top">
      <div>
        <h2 style="margin:0;">Kelola Owner</h2>
        <div style="color:var(--muted); font-size:13px;">Suspend / Activate / Soft Delete akun OWNER</div>
      </div>
    ...
    %>
      <div style="color:var(--muted);">Tidak ada akun OWNER.</div>
    <%
      } else {
    %>
    <table>
      ...
      <% for (user o : owners) {
           String st = "-";
           try { st = o.getStatus(); } catch(Exception ignore){}
      %>
      <tr>
        <td><%= o.getId() %></td>
        <td><%= o.getNama() %></td>
        ...
        <td>
          <form method="post" action="<%= request.getContextPath() %>/admin/owners/action" style="display:inline;">
            <input type="hidden" name="ownerId" value="<%= o.getId() %>"/>
          ...
          <form method="post" action="<%= request.getContextPath() %>/admin/owners/action" style="display:inline;">
            ...
            <button class="danger" type="submit" name="action" value="delete"
              onclick="return confirm('Soft delete owner ini?');">
              Soft Delete
            </button>
          </form>
        </td>
